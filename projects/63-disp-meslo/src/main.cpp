...
void drawMesloShowcase(TextCanvas& canvas)
{
    ...
    drawCenteredText(canvas, Meslo16, "MESLO NERD FONT | 1-BIT SUBSET", 24);
    canvas.drawFastHLine(20, 35, canvas.width() - 40, COLOR_BLACK);

    drawText(canvas, Meslo16, "36 PX", 16, 76);
    drawCenteredText(canvas, Meslo36, "Straße & Grüße", 77);

    drawText(canvas, Meslo16, "24 PX", 16, 119);
    drawCenteredText(canvas, Meslo24, "ÄÖÜ äöü ß | 0123456789", 120);

    drawText(canvas, Meslo16, "16 PX", 16, 151);
    drawCenteredText(canvas, Meslo16, "Fahrplan: nächste Tram in 6 Minuten", 151);
    canvas.drawFastHLine(20, 169, canvas.width() - 40, COLOR_BLACK);

    for (size_t index = 0; index < 5; ++index)
    {
        drawCenteredIcon(canvas, ICONS[index], ICON_CENTERS[index], 214);
        drawCenteredLabel(canvas, LABELS[index], ICON_CENTERS[index], 249);
    }
}
...
void setup()
{
    ...
    Serial.println();
    Serial.println("CrowPanel 5.79-inch Meslo font test");
    ...
    if (refreshDisplay())
    {
        Serial.println("Meslo-Test abgeschlossen; Display ist im Deep Sleep.");
    }
}
