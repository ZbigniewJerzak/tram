...
constexpr int16_t DISPLAY_FOOTER_BASELINE_OFFSET = 9;

// Calibration derived from the photographed panel: framebuffer content appears
// about 16 logical pixels too far right. Change only this value if a final
// adjustment is needed, for example -14 or -18.
constexpr int16_t DISPLAY_X_CORRECTION = -16;
...
void drawFullWidthLine(int16_t y) {
  display.drawFastHLine(
      displayX(0),
      y,
      display.width() - DISPLAY_X_CORRECTION,
      GxEPD_BLACK);
}
...
void drawRightAligned(const String& text, int16_t rightX,
  ...
  display.getTextBounds(text.c_str(), 0, y, &x1, &y1, &width, &height);
...
void drawDepartureScreen(bool forceFullRefresh = false) {
  ...
  do {
    ...
    // Header: stop name, current time and data timestamp.
    display.setTextSize(1);
    display.setCursor(displayX(4), 2);
    display.print(clipped(stopDisplayName, 25));

    if (!lastError.isEmpty()) {
      drawRightAligned("!", 246, 2, 1);
    }

    display.setTextSize(3);
    display.setCursor(displayX(4), 12);
    display.print(hhmm(now));

    display.setTextSize(1);
    display.setCursor(displayX(108), 16);
    display.print(TXT_NEXT_TRAMS);
    display.setCursor(displayX(108), 27);
    display.print(String(TXT_DATA_AS_OF) + hhmm(lastSuccessfulUpdate));

    drawFullWidthLine(40);

    if (cachedDepartureCount <= 0) {
      display.setTextSize(2);
      display.setCursor(displayX(4), 57);
      display.print(TXT_NO_DEPARTURES);

      display.setTextSize(1);
      display.setCursor(displayX(4), 82);
      display.print(TXT_RETRYING);
    } else {
      for (int i = 0; i < MAX_DISPLAY_DEPARTURES; ++i) {
        const int16_t rowTop = 45 + i * 36;

        if (i >= cachedDepartureCount) {
          display.setTextSize(1);
          display.setCursor(displayX(4), rowTop);
          display.print("--");
          continue;
        }
        ...
        String metadata = cachedDepartures[i].line + "  " +
                          hhmm(cachedDepartures[i].when);
        if (cachedDepartures[i].delayMinutes != 0) {
          metadata += "  ";
          if (cachedDepartures[i].delayMinutes > 0) metadata += "+";
          metadata += String(cachedDepartures[i].delayMinutes);
        }

        display.setTextSize(1);
        display.setCursor(displayX(4), rowTop);
        display.print(clipped(metadata, 27));

        display.setTextSize(2);
        display.setCursor(displayX(4), rowTop + 10);
        display.print(shortDirection(cachedDepartures[i].direction));

        drawRightAligned(String(minutes) + TXT_MINUTES, 246, rowTop + 10, 2);

        if (i == 0) {
          drawFullWidthLine(77);
        }
      }
    }

    // Errors appear only in the bottom line; old departure data remains visible.
    if (!lastError.isEmpty()) {
      display.setTextSize(1);
      const int16_t footerY =
          display.height() - DISPLAY_FOOTER_BASELINE_OFFSET;
      display.setCursor(displayX(4), footerY);
      display.print(clipped(
          String(TXT_ERROR_PREFIX) + lastError + TXT_STALE_DATA, 39));
    }
  } while (display.nextPage());
}
