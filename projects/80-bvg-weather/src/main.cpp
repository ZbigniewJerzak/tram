...
constexpr int16_t BVG_CONTENT_LEFT = 12;
constexpr int16_t BVG_CONTENT_RIGHT = 516;
constexpr int16_t BVG_HEADER_BOTTOM = 43;
constexpr int16_t BVG_FIRST_ROW_BOTTOM = 157;
...
BvgDepartureClient bvg(BVG_STOP_SEARCH_TERM);
...
BvgDepartureBoard departureBoard;
...
String clockText(const time_t value)
{
    if (value <= 0)
    {
        return "--:--";
    ...
    char output[6] = {};
    strftime(output, sizeof(output), "%H:%M", &localTime);
    return output;
}
...
)
{
        ...
        canvas.textWidth(Spleen16x32, text + ELLIPSIS) > maximumWidth)
    {
        ...
        while (removeAt > 0 &&
            (static_cast<uint8_t>(text[removeAt]) & 0xC0U) == 0x80U)
        {
            --removeAt;
        }
...
{
    long remainingSeconds = departure.when - now;
    if (remainingSeconds < 0)
    {
        remainingSeconds = 0;
    }
    const int remainingMinutes = static_cast<int>((remainingSeconds + 59) / 60);
...
{
    canvas.setClipRect(0, 0, BVG_PANEL_WIDTH, canvas.height());
    canvas.drawRect(0, 0, BVG_PANEL_WIDTH + 1, canvas.height(), COLOR_BLACK);
    canvas.drawFastHLine(0, BVG_HEADER_BOTTOM, BVG_PANEL_WIDTH, COLOR_BLACK);
    canvas.drawFastHLine(0, BVG_FIRST_ROW_BOTTOM, BVG_PANEL_WIDTH, COLOR_BLACK);
    canvas.drawText(
        Spleen16x32,
        "ERICH-BARON-WEG",
        BVG_CONTENT_LEFT,
    ...
    {
        canvas.drawText(Spleen16x32, "--:--", BVG_CONTENT_LEFT, 199);
    }
}
...
void setup()
{
    ...
    delay(1000);
    Serial.println();
    Serial.println("CrowPanel 5.79-inch BVG and weather dashboard");
    Serial.println("BVG/HomePilot: 60 s | OpenWeather: 600 s");
